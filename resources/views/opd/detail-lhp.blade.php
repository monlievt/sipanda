                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase
                                    ...
                                    {{ in_array($item->status_telaah, ['diajukan_irban', 'diajukan_inspektur']) || $item->status_tindak_lanjut === 'menunggu_verifikasi' ? 'bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-300' : '' }}
                                    {{ in_array($item->status_telaah, ['revisi_irban', 'revisi_inspektur']) ? 'bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300' : '' }}
                                    {{ in_array($item->status_tindak_lanjut, ['belum', 'proses']) && !in_array($item->status_telaah, ['diajukan_irban', 'diajukan_inspektur']) ? 'bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300' : '' }}
                                    {{ $item->status_tindak_lanjut === 'tdt' ? 'bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-300' : '' }}">
                                    @if(in_array($item->status_telaah, ['diajukan_irban', 'diajukan_inspektur']))
                                        Sedang Diverifikasi APIP (Telaah Berjenjang)
                                    @else
                                        {{ $item->status_label }}
                                    @endif
                                </span>
