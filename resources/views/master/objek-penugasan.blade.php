<x-app-layout>
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Daftar Perangkat Daerah, Kecamatan, & Objek Pemeriksaan di Kabupaten Trenggalek.</p>
        </div>

        <button onclick="document.getElementById('modalTambahObjek').classList.remove('hidden')" class="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs rounded-xl shadow-lg shadow-emerald-600/20 transition-all flex items-center gap-2">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="py-8 text-center text-slate-400">Belum ada objek penugasan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="modalTambahObjek" class="hidden fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
        <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl max-w-md w-full p-6 border border-slate-200 dark:border-slate-800 text-xs">
            <div class="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
                ...
                <button onclick="document.getElementById('modalTambahObjek').classList.add('hidden')" class="text-slate-400 hover:text-slate-600 text-xl font-bold">&times;</button>
            </div>
            ...
            <form method="POST" action="{{ route('master.objek-penugasan.store') }}" class="space-y-4 mt-4">
                ...
                <div>
                    ...
                    <input type="text" name="nama" required placeholder="mis. Dinas Pendidikan / Desa Bendorejo" class="w-full rounded-xl border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs">
                </div>
                ...
                <div class="pt-4 flex items-center justify-end gap-3 border-t border-slate-200 dark:border-slate-800">
                    <button type="button" onclick="document.getElementById('modalTambahObjek').classList.add('hidden')" class="px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-300 font-semibold rounded-xl">Batal</button>
                    <button type="submit" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl shadow-md">Simpan Objek</button>
                </div>
            </form>
        </div>
    </div>
