...
class ImportController extends Controller
{
    ...
    /**
     * Unduh Template CSV Standar untuk Import.
     */
    public function template(string $type): StreamedResponse|RedirectResponse
    {
        $templates = [
            'penugasan' => [
                'filename' => 'Template_Import_Penugasan_SPT.csv',
                'headers'  => [
            ...
            'tindak_lanjut' => [
                'filename' => 'Template_Import_Matriks_LHP.csv',
                'headers'  => [
            ...
            'objek' => [
                'filename' => 'Template_Import_Objek_Pengawasan.csv',
    ...
    /**
     * Preview File CSV sebelum Eksekusi Import.
     */
    public function preview(Request $request): View|RedirectResponse
    {
        ...
        $rows = $this->importService->parseCsv($file);

        if (empty($rows)) {
            return back()->with('error', 'File CSV kosong atau format tidak dapat dibaca.');
        }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($request->hasFile('file')) {
            $rows = $this->importService->parseCsv($request->file('file'));
